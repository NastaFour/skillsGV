...
name: pytest
description: >
  Python testing patterns with pytest. Fixtures, mocking, parametrize, markers, async tests,
  and conftest.py structure. Trigger when the user writes or debugs Python tests, uses pytest,
  or asks about test coverage, fixtures, or mocking in a Python project.
license: MIT
...
metadata:
  ...
  version: "1.0"
  category: 07-testing
  tags: [pytest, python, testing, fixtures, mocking, parametrize, async]
---
...
# ✅ DO: one behavior per test, use fixtures
def test_create_user_sets_name(user):
...
## Integration with skillsGV

Se activa automáticamente cuando detectes:
- Archivos `test_*.py` o `*_test.py`
- Imports de `pytest`, `unittest.mock`, `pytest.fixture`
- El usuario menciona "tests en Python", "fixtures", "mocking"

Combinar con: `testing-patterns` (patrones generales), `docker` (test en contenedor), `ci-cd` (pipeline de CI).
