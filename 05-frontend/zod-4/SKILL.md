---
name: zod-4
description: Zod 4 schema validation patterns with breaking changes from v3 — top-level validators (z.email/z.uuid/z.url), 'error' param instead of 'message', object error config, React Hook Form integration. Use when validating with Zod so the AI does not regress to v3 APIs.
license: Apache-2.0
compatibility: "Compatible with Claude Code, OpenCode, Cursor, Copilot, Codex. Requires Zod 4+."
allowed-tools: Bash(node:*) Read
metadata:
  trigger: ["zod 4", "zod validation", "schema validation", "react hook form", "zodResolver", "safeParse", "discriminated union"]
...
const scoresSchema = z.record(z.string(), z.number()); // { [k: string]: number }
const coordinatesSchema = z.tuple([z.number(), z.number()]); // [number, number]
```
...
const customSchema = z.string({
  error: (issue) => issue.code === "too_small" ? "String is too short" : "Invalid string",
});
```
