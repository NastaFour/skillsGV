---
name: ai-sdk-5
description: Vercel AI SDK 5 patterns with breaking changes from v4 — useChat from @ai-sdk/react, DefaultChatTransport, UIMessage parts array, route handler streaming, tools, LangChain integration. Use when building AI chat features so the AI does not regress to v4 import paths and string-content messages.
license: Apache-2.0
compatibility: "Compatible with Claude Code, OpenCode, Cursor, Copilot, Codex. Requires AI SDK 5+, Node 20+."
allowed-tools: Bash(node:*) Read
metadata:
  trigger: ["ai sdk 5", "vercel ai sdk", "useChat", "DefaultChatTransport", "UIMessage", "streamText", "ai chat", "llm streaming"]
  ...
  return (
    <div className={message.role === "user" ? "user" : "assistant"}>
      {message.parts.map((part, index) => {
        ...
        if (part.type === "image") return <img key={index} src={part.image} alt="" />;
        return null;
      })}
    </div>
  );
...
const { error, messages, sendMessage } = useChat({
  transport: new DefaultChatTransport({ api: "/api/chat" }),
  onError: (error) => { console.error("Chat error:", error); toast.error("Failed to send message"); },
});
{error && (
  <div className="error">
    {error.message}
    <button onClick={() => sendMessage({ text: lastInput })}>Retry</button>
  </div>
)}
```
