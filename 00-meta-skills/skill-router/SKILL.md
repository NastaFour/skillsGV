...
> Two-tier loading: this skill is **Tier 0** (always loaded, ~2K tokens). It tells the agent which **Tier 1** skills to load on-demand, so Tier 0 stays small instead of inflating to 209 skills × ~150 tokens.
...
`tier1toLoad` tells the agent exactly which Tier 1 skill bodies to read. 1-3 skills, not 192.
