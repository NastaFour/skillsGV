...
1. **Autoridad de review nativa** — estado estructurado, recibo terminal y contexto del gate de review (pre-archive), si existen. Hechos de entrega validados; ganan en todo lo que cubren. El pipeline es `sdd-verify` → gate de recibo → `sdd-archive`: con el modo review activo (opt-in, propiedad del usuario vía `gentle-ai review mode enable --scope global`), el recibo validado por la capa nativa gobierna el cierre; con el modo inactivo no se exige recibo y aplica la política ordinaria. Contrato: `_shared/review-ledger-contract.md`.
